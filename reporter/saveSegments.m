...
%
%% saveSegments
% Function: Save the segments of a segmentation in a file
% 
% Input:
...
%     
% Output:
%     No output variables. Only a file which contains the saved segments
function saveSegments(model, dataset, repsuffix)
    
    cuts = model.cuts;
    nOfCuts = size(cuts,2);
    cutscero = [1 cuts length(model.segmentation)];
    class = model.L;
    centroids = model.C;
    ...
    f = fopen([outputFile '_segments.csv'], 'wt');
    for i=1:nOfCuts+1,
        fprintf(f, '%d;%d;', i, cutscero(i));
        for j=1:nOfFeatures,
            fprintf(f, '%f;', model.features(i,j));
        end
        fprintf(f, '%d\n', class(i));
    end
    fclose(f);
