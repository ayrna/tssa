%NHTSS Clustering - Normal Time series segmentation [1]
%
%   NHTSS methods:
%      runAlgorithm               - runs the corresponding algorithm and its hybrid versions (GA and GA+LS in [1])
...
classdef NHTSS < handle
    
    properties
        name_parameters = {'numIt','nPobl','k','pCross','pMut','seed','minSeg','maxSeg','sizeChromosome','iterClust','polyDegree','characActivation','umbralEntropy'}
        dataFile
        data
        parameters
    end
    ...
    methods
        ...
        function obj = defaultParameters(obj)
            ...
            % umbral for normal hybridization
        ...
        function [information] = runAlgorithm(obj, serie)
            ...
            for i=1:obj.parameters.numIt,
                ...
                %'Mutation'
                [newPopulation, newFitness] = mutation1(newPopulation,newFitness,obj.parameters.pMut,...
            ...
            chromosome = hybridIndividualNormal(currentPopulation(fbestidxGA,:),obj.data,obj.parameters.umbralEntropy,obj.parameters.minSeg);
