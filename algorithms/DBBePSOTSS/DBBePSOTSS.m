 classdef DBBePSOTSS < handle
    %DBBePSOTSS Approximation DBBePSO Time series segmentation [1]
    %
    %   DBBePSOTSS methods:
    %      runAlgorithm               - runs the corresponding algorithm and its hybrid versions (DBBePSO and HDBBePSO in [1])
    ...
    methods
        ...
        function [information] = runAlgorithm(obj, serie)
            ...
            %'Initial velocities'
            %initEvaluations=numberEvaluations;
            tic;
            for i=1:obj.parameters.numIt,
                %'PSO'
                factor = ((0.9 * (obj.parameters.numIt - i))/(obj.parameters.numIt - 1)) + 0.1;
                [currentPopulationInt, newFitness] = updatePositionsDBB(currentPopulationInt,bestLocalPopulationInt,bestIndividualInt,factor,currentFitness,...
                ...
                %'Update arrays'
                [bestLocalPopulationInt, bestLocalFitness, bestIndividualInt, bestIndividualFitness] = updateArrays(currentPopulationInt, currentFitness, bestLocalPopulationInt, bestLocalFitness, bestIndividualInt, bestIndividualFitness);
