...
%
%% clusteringKmeans
% Function: Function:returns the assignation and centroids of a set of patterns
% Input:
%     normCharac: Set of pattens (in our case, set of characteristics)
%     k:          Number of groups
%     iterClust:  Max iterations for k-means
% 
% Output:
%     assignation: Array of group assignation for each pattern (segment)
%     centroids:   Set of centroids resulting of the clustering
function [assignation, centroids] = clusteringKmeans(normCharac,k,iterClust)
    C = initCentroids(normCharac,k);
    [assignation, centroids] = dcKMeans(normCharac,k,C,iterClust);
end
